...
{
    public:
    gimbal_control(const string& gimbal_topic_orientation, const string& gimbal_topic_command):
        nh("~")
    {
        // 订阅云台当前角度
        gimbal_att_sub = nh.subscribe<geometry_msgs::Quaternion>(gimbal_topic_orientation, 10, &gimbal_control::gimbal_att_cb,this);

        // 云台控制：本话题要发送至飞控(通过Mavros_extra功能包 /plugins/mount_control.cpp发送)
        mount_control_pub = nh.advertise<mavros_msgs::MountControl>(gimbal_topic_command, 1);
...
float gimbal_control::constrain_function2(float data, float Min,float Max)
{
    if(data > Max)
    {
        return Max;
